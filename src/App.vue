<script setup>
  ...
  onMounted(() => {
    fetch("https://backend-colores-intensivo-9t6u.onrender.com/colores")
    .then(respuesta => respuesta.json())
    .then(respuestaProcesada => colores.value = respuestaProcesada)   
  })

  function crearColor(color){
    fetch("https://backend-colores-intensivo-9t6u.onrender.com/colores/nuevo", {
      method: "POST",
  ...
  function borrarColor(id){
    fetch(`https://backend-colores-intensivo-9t6u.onrender.com/colores/borrar/${id}`,{
      method: "DELETE"
    })
    .then(respuesta => respuesta.json())
    .then(({error}) => {
